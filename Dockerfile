...
#RUN if [ "$TARGETPLATFORM" = "linux/arm64" ]; then GOARCH='arm' GOHOSTARCH='arm' CC=arm-linux-gnueabihf-gcc GOGCCFLAGS="-march=armv8-a" CXX=arm-linux-gnueabi-g++ go build -gcflags="-l -N" -o ./mediasink; else go build -o ./main ; fi

RUN go build -o ./main -ldflags="-X 'main.Version=$VERSION' -X 'main.Commit=$COMMIT'"
