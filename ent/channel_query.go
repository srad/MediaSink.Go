...
func (cq *ChannelQuery) QueryRecordings() *RecordingQuery {
	query := (&RecordingClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, channel.RecordingsTable, channel.RecordingsColumn),
		)
...
func (cq *ChannelQuery) QueryJobs() *JobQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, channel.JobsTable, channel.JobsColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *ChannelQuery) Clone() *ChannelQuery {
	...
	return &ChannelQuery{
		...
		withRecordings: cq.withRecordings.Clone(),
...
func (cq *ChannelQuery) WithRecordings(opts ...func(*RecordingQuery)) *ChannelQuery {
	query := (&RecordingClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withRecordings = query
	return cq
}
...
func (cq *ChannelQuery) WithJobs(opts ...func(*JobQuery)) *ChannelQuery {
	query := (&JobClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(channel.FieldChannelName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Channel.Query().
//		Select(channel.FieldChannelName).
//		Scan(ctx, &v)
...
func (cq *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	if query := cq.withRecordings; query != nil {
		if err := cq.loadRecordings(ctx, query, nodes,
			func(n *Channel) { n.Edges.Recordings = []*Recording{} },
			func(n *Channel, e *Recording) { n.Edges.Recordings = append(n.Edges.Recordings, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withJobs; query != nil {
		if err := cq.loadJobs(ctx, query, nodes,
			func(n *Channel) { n.Edges.Jobs = []*Job{} },
			func(n *Channel, e *Job) { n.Edges.Jobs = append(n.Edges.Jobs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChannelQuery) loadRecordings(ctx context.Context, query *RecordingQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Recording)) error {
	...
	query.Where(predicate.Recording(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(channel.RecordingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.channel_recordings
		if fk == nil {
			return fmt.Errorf(`foreign-key "channel_recordings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "channel_recordings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *ChannelQuery) loadJobs(ctx context.Context, query *JobQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Job)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Channel)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Job(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(channel.JobsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.channel_jobs
		if fk == nil {
			return fmt.Errorf(`foreign-key "channel_jobs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "channel_jobs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
